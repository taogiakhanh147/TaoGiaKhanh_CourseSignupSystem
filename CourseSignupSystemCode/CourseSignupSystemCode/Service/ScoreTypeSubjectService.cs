...
namespace CourseSignupSystemCode.Service
{
    public class ScoreTypeSubjectService : IScoreTypeSubjectService
    {
        ...
        public async Task<List<GetAllSubjectsDTO>> GetAllSubjectsAsync()
        {
            var getAllSubjects = await _context.ScoreTypeSubjects
                               .Where(s => s.IDSubject == s.SubjectOfStudent.ID 
                                      && s.IDCourse == s.SubjectOfStudent.Class.Course.ID
                                      && s.IDScoreType == s.ScoreType.ID)
                               .Select(s => new GetAllSubjectsDTO
                               {
                                   GradingColumn = s.GradingColumn,
                                   RequiredGradeColumns = s.RequiredGradeColumns,
                                   CourseName = s.SubjectOfStudent.Class.Course.CourseName,
                                   SubjectName = s.SubjectOfStudent.SubjectName,
                                   ScoreTypeName = s.ScoreType.ScoreTypeName
                               })
                               .ToListAsync();
